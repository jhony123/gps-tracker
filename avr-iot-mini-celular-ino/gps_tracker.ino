...
#define HTTP_DOMAIN "location-collector-api-node-production.up.railway.app"
...
static void initializeHTTP(void) {
    if (!HttpClient.configure(HTTP_DOMAIN, 443, true)) {
...
static void sendData(void) {
    ...
    sprintf_P(data,
              PSTR("{\"lat\":\"%s\",\"lon\":\"%s\",\"time\":\"%s\",\"id\":<insert id here>,\"pass\":\"<insert pass here>\",\"vol\":\"%f\",\"speed\":\"%s\"}"),
              latitude,
              longitude,
              time,
              voltage,
    ...
    HttpResponse response = HttpClient.post("/location", data);
